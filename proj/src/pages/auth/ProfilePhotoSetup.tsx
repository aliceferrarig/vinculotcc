...
export function ProfilePhotoSetup(){
  ...
  return <Card className="p-6 text-center sm:p-9"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-sage-100 text-sage-700"><Camera size={22}/></span><h1 className="mt-5 text-2xl font-semibold text-sage-700">{isPsychologist?'Adicione sua foto profissional':'Quer adicionar uma foto?'}</h1><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-sage-500">{isPsychologist?'Uma boa foto ajuda os clientes a reconhecerem e confiarem em você. É opcional e pode ser adicionada depois.':'A foto deixa sua experiência mais pessoal, mas você também pode fazer isso depois.'}</p>
    ...
    <button type="button" disabled={!profile||loading} onClick={destination} className="mt-4 text-sm font-semibold text-sage-500 hover:text-sage-700">Pular por enquanto</button>
    <PhotoAdjuster file={editingFile} onCancel={()=>setEditingFile(null)} onConfirm={adjusted=>{setFile(adjusted);setEditingFile(null)}}/>
  </Card>
}

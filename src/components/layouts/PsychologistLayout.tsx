import { Bell, CalendarDays, ClipboardList, CreditCard, LayoutDashboard, LogOut, Menu, Settings, Star, UserRound, UsersRound, X } from 'lucide-react'
...
const items=[
  ...
  ['/psicologo/avaliacoes','Avaliações',Star], ['/psicologo/plano','Plano e assinatura',CreditCard], ['/psicologo/configuracoes','Configurações',Settings]
] as const
...
export function PsychologistLayout(){
  ...
  return <div className="min-h-screen bg-cream lg:flex">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col bg-white p-5 transition-transform lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${open?'translate-x-0':'-translate-x-full'}`}>
      ...
      <div className="leaf -bottom-6 left-10 scale-75"/><button onClick={()=>navigate('/')} className="relative mt-auto flex items-center gap-3 border-t border-sage-100 px-3 pt-5 text-sm"><LogOut size={19}/>Sair</button>
    </aside>
